...
export interface RadiographAcquisitionProps {
  previewUrl: string | null;
  handleImageUpload: (e: React.ChangeEvent<HTMLInputElement>) => void;
  removeImage: () => void;
}

export function RadiographAcquisition({
  previewUrl,
  handleImageUpload,
  removeImage,
}: RadiographAcquisitionProps) {
        ...
        {previewUrl ? (
          ...
          <div className="p-4 space-y-4 pointer-events-none">
            <div className="w-20 h-20 rounded-full bg-default-100 flex items-center justify-center mx-auto mb-2 border-2 border-dashed border-default-200">
              <UploadCloud
                size={36}
                className={`transition-colors ${isDragging ? "text-primary" : "text-default-400"}`}
              />
            </div>
