...
class LungPreprocessor:
    ...
    def remove_artifacts_and_segment(self, image):
        ...
        segmented = cropped * mask_float + cropped * 0.2 * (
            1 - mask_float
        )  # Keep 20% background context
    ...
    def normalize_intensity(self, image):
        ...
        image = (
            (image - image.min()) / (max(1e-8, image.max() - image.min())) * 255
        ).astype(np.uint8)
        return image
...
def get_train_transforms(image_size=224):
    ...
    return A.Compose(
        [
            A.RandomResizedCrop(
                size=(image_size, image_size), scale=(0.85, 1.0), p=0.8
            ),
